...
### 🔧 变更（Changed）
- 统一版本号读取自根目录.version并同步前后端配置
- 升级mihomo配置前自动备份config.yaml
- 合并设置页更新与系统信息并新增版本历史
- 统一移动端和桌面端动画速度
- 重新设计顶部导航栏用户菜单
- 全面美化登录界面
...
- 自动同步版本配置到 Tauri 并清理生成文件

### 🐛 修复（Fixed）
- 修复更新资产选择区分glibc与musl
- 修复日志入口导致页面重载
- 修复侧边栏点击日志后滚动位置重置
- 修复Mihomo配置升级无损合并用户自定义键
- 修复SSE日志时间解析优先使用time字段
- 修复日志时间解析显示1970的问题
- 日志时间戳改为ISO8601字符串
- 修复移动端动画被全局规则覆盖的问题
- 优化移动端 logo 动画性能
...
- 修复 sysctl 写入补换行校验
- 升级后仅在运行时重启 mosdns 服务
- 未初始化时跳过 mosdns 配置升级

### ⚠️ 废弃（Deprecated）
- 移除已迁移方案包残留
- 移除登录页未使用的组件引用
- 移除 migrated timezone plan
...
::: details 📋 构建信息
- **源提交**: [`73f4e12`](https://github.com/msm9527/msm/commit/73f4e12443fa336ae157b1087752e563a1efb4fb)
- **提交信息**: update 0.7.6
- **提交作者**: doumao
- **提交时间**: 2026-01-10 18:08:56 +0800
:::
